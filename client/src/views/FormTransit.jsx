...
const FormTransit = () => {
        ...
        axios.get(endpoint)
            .then(res => {
                ...
                if (res.data && Array.isArray(res.data)) {
                    setListaAnimales(res.data.map(animal => ({
    ...
    const validateStep = () => {
        ...
        if (currentStep === 3) {
            if (!data.tiempoDisponible.trim()) newErrors.tiempoDisponible = "Indica cuánto tiempo podés tener al animal";
            if (!data.experienciaConAnimales.trim()) newErrors.experienciaConAnimales = "Indica tu experiencia con animales";
            if (!data.cuidadosEspecialesPosibles.trim()) newErrors.cuidadosEspecialesPosibles = "Indica si podés realizar cuidados especiales";
            if (!data.otrasMascotas.trim()) newErrors.otrasMascotas = "Indica si tenés otras mascotas";
        }
        
        if (currentStep === 4) {
            if (!data.horariosDisponibles.trim()) newErrors.horariosDisponibles = "Indica tus horarios disponibles";
            if (!data.experienciaEspecifica.trim()) newErrors.experienciaEspecifica = "Describí tu experiencia específica";
            if (!data.contactoEmergencia.trim()) newErrors.contactoEmergencia = "Proporcioná un contacto de emergencia";
        }
        
        if (currentStep === 5) {
            if (!data.compromiso.trim()) newErrors.compromiso = "Este campo es requerido";
            if (!data.referencias.trim()) newErrors.referencias = "Podés incluir referencias o comentarios adicionales";
        }
    ...
    const sendData = async (e) => {
        ...
        for (let step = 1; step <= totalSteps; step++) {
            ...
            if (step === 1) {
                ...
                if (!data.apellido.trim()) stepErrors.apellido = "El apellido es requerido";
                if (!data.edad || data.edad < 18 || data.edad > 100) stepErrors.edad = "Debes ser mayor de 18 años";
                if (!data.zona.trim()) stepErrors.zona = "La zona es requerida";
                if (!data.direccion.trim()) stepErrors.direccion = "La dirección es requerida";
                if (!data.telefono.trim() || data.telefono.length < 8) stepErrors.telefono = "El teléfono es requerido";
                if (!data.mail.trim() || !data.mail.includes("@")) stepErrors.mail = "Email inválido";
            }
            
            if (step === 2) {
                if (!data.adoptarA.trim()) stepErrors.adoptarA = "Selecciona un animal";
                if (!data.animalId) stepErrors.adoptarA = "Debes seleccionar un animal de la lista";
                if (!data.motivo.trim() || data.motivo.trim().length < 5) stepErrors.motivo = "Explica tu motivación (mínimo 5 caracteres)";
            }
            
            if (step === 3) {
                if (!data.tiempoDisponible.trim()) stepErrors.tiempoDisponible = "Indica cuánto tiempo podés tener al animal";
                if (!data.experienciaConAnimales.trim()) stepErrors.experienciaConAnimales = "Indica tu experiencia con animales";
                if (!data.cuidadosEspecialesPosibles.trim()) stepErrors.cuidadosEspecialesPosibles = "Indica si podés realizar cuidados especiales";
                if (!data.otrasMascotas.trim()) stepErrors.otrasMascotas = "Indica si tenés otras mascotas";
            }
            
            if (step === 4) {
                if (!data.horariosDisponibles.trim()) stepErrors.horariosDisponibles = "Indica tus horarios disponibles";
                if (!data.experienciaEspecifica.trim()) stepErrors.experienciaEspecifica = "Describí tu experiencia específica";
                if (!data.contactoEmergencia.trim()) stepErrors.contactoEmergencia = "Proporcioná un contacto de emergencia";
            }
            
            if (step === 5) {
                if (!data.compromiso.trim()) stepErrors.compromiso = "Confirma tu compromiso";
                if (!data.referencias.trim()) stepErrors.referencias = "Incluye referencias o comentarios";
            }
        ...
        try {
            ...
            setData({
                ...
                mail: "", adoptarA: "", animalId: "", motivo: "",
                tiempoDisponible: "", experienciaConAnimales: "", cuidadosEspecialesPosibles: "",
                cubrirGastos: "si", espacioEnCasa: "mediano", otrasMascotas: "",
                horariosDisponibles: "", experienciaEspecifica: "", contactoEmergencia: "",
                referencias: "", compromiso: ""
            });
            ...
            if (error.response) {
                ...
                if (error.response.status === 400) {
                    ...
                    setGeneralError("Animal no encontrado. Por favor, selecciona otro animal.");
                    setCurrentStep(2);
                } else if (error.response.status === 409) {
                    setGeneralError("Ya has enviado una solicitud para este animal");
                } else if (error.response.status === 500) {
                    setGeneralError("Error interno del servidor. Por favor, intenta más tarde.");
                } else {
                    setGeneralError(`Error ${error.response.status}: ${responseData.message || "Error desconocido"}`);
                }
        ...
        return (
            <div className={styles.section}>
                ...
                <div className="position-relative mb-4">
                    <label className="title_orange mx-2 mb-1">¿A cuál de nuestros rescatados querés dar tránsito? *</label>
                ...
                <div className="d-flex flex-column mb-4">
                    ...
                    <textarea 
                        ...
                        rows={3}
                        className={`${styles.textarea} ${errors.motivo ? styles.errorInput : ""}`}
                    ></textarea>
    ...
    const renderStep3 = () => (
        <div className={styles.section}>
            ...
            <div className="d-flex flex-column mb-4">
                <label className="title_orange mx-2 mt-1 mb-1">¿Cuánto tiempo podés tener al animal? *</label>
                <textarea 
                    ...
                    rows={2}
                    className={`${styles.textarea} ${errors.tiempoDisponible ? styles.errorInput : ""}`}
                ></textarea>
                {errors.tiempoDisponible && <span className={styles.errorText}>{errors.tiempoDisponible}</span>}
            </div>
            ...
            <div className="d-flex flex-column mb-4">
                <label className="title_orange mx-2 mt-1 mb-1">¿Podés realizar cuidados especiales si son necesarios? *</label>
                <textarea 
                    ...
                    rows={2}
                    className={`${styles.textarea} ${errors.cuidadosEspecialesPosibles ? styles.errorInput : ""}`}
                ></textarea>
                {errors.cuidadosEspecialesPosibles && <span className={styles.errorText}>{errors.cuidadosEspecialesPosibles}</span>}
            </div>
            ...
            <div className="row">
                <div className="col-md-6">
                    <div className="d-flex flex-column mb-4">
                        <label className="title_orange mx-2 mt-1 mb-1">¿Podés cubrir gastos básicos? *</label>
                        <select 
                            name="cubrirGastos" 
                ...
                <div className="col-md-6">
                    <div className="d-flex flex-column mb-4">
                        <label className="title_orange mx-2 mt-1 mb-1">Espacio disponible en tu vivienda *</label>
                        <select 
                            name="espacioEnCasa" 
        ...
        <div className={styles.section}>
            ...
            <div className="d-flex flex-column mb-4">
                ...
                <textarea
                    ...
                    rows={2}
                    className={`${styles.textarea} ${errors.horariosDisponibles ? styles.errorInput : ""}`}
                ></textarea>
                {errors.horariosDisponibles && <span className={styles.errorText}>{errors.horariosDisponibles}</span>}
            </div>
            ...
            <div className="d-flex flex-column mb-4">
                ...
                <textarea
                    ...
                    rows={2}
                    className={`${styles.textarea} ${errors.experienciaEspecifica ? styles.errorInput : ""}`}
                ></textarea>
                {errors.experienciaEspecifica && <span className={styles.errorText}>{errors.experienciaEspecifica}</span>}
            </div>
            ...
            <div className="d-flex flex-column mb-4">
                ...
                <textarea
                    ...
                    rows={2}
                    className={`${styles.textarea} ${errors.contactoEmergencia ? styles.errorInput : ""}`}
                ></textarea>
                {errors.contactoEmergencia && <span className={styles.errorText}>{errors.contactoEmergencia}</span>}
            </div>
        </div>
    );
        ...
        <div className={styles.section}>
            ...
            <div className="d-flex flex-column mb-4 mt-3">
                ...
                <textarea 
                    ...
                    rows={3}
                    className={`${styles.textarea} ${errors.referencias ? styles.errorInput : ""}`}
                ></textarea>
                {errors.referencias && <span className={styles.errorText}>{errors.referencias}</span>}
            </div>
            ...
            <div className="alert alert-warning mt-4">
                ...
                <p className="mb-0">
                    Como hogar transitorio, te comprometes a cuidar temporalmente al animal hasta que encuentre 
                    un hogar definitivo. La fundación proporcionará apoyo y seguimiento durante el proceso.
                </p>
            </div>
        </div>
    );
    ...
    return (
        <div className={styles.wrapper}>
            ...
            <p className="text-center mb-4">
                Completa este formulario para postularte como hogar de tránsito. 
