...
const Form = () => {
  ...
  const [data, setData] = useState({
    ...
    // Sobre la adopción/tránsito
    adoptarA: "",
    animalId: "",
    adoptarOtro: "",
    motivoAdopcion: "", // ✅ CORREGIDO: Usar motivoAdopcion directamente
  ...
  // Cargar lista de animales según el tipo
  useEffect(() => {
    ...
    console.log("🌐 Cargando animales desde:", endpoint);
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data && Array.isArray(res.data)) {
          const animalesFiltrados = res.data
            .filter((animal) => animal.estadoGeneral === true)
            .map((animal) => ({
          ...
          console.log("📊 Animales disponibles:", animalesFiltrados.length);
          setListaAnimales(animalesFiltrados);
        }
        setLoadingAnimales(false);
      })
      .catch((err) => {
        console.error(`❌ Error cargando animales para ${formType}:`, err);
        setLoadingAnimales(false);
        setListaAnimales([]);
      });
  }, [formType]);
  ...
  const validateStep = () => {
    ...
    if (currentStep === 2) {
      ...
      if (!data.adoptarOtro.trim() && formType === "adopcion")
        newErrors.adoptarOtro = "Responde esta pregunta";

      // ✅ VALIDACIÓN CORREGIDA para motivoAdopcion
      if (!data.motivoAdopcion.trim()) {
        ...
        newErrors.motivoAdopcion =
          "El motivo debe tener al menos 10 caracteres";
      }
    }

    if (currentStep === 3) {
      if (!data.convivientes.trim())
        newErrors.convivientes = "Este campo es requerido";
      ...
      if (!data.otrasMascotas.trim())
        newErrors.otrasMascotas = "Este campo es requerido";
    }

    if (currentStep === 4) {
      if (
        !data.horasFueraDeCasa ||
        data.horasFueraDeCasa < 0 ||
        data.horasFueraDeCasa > 24
      )
        newErrors.horasFueraDeCasa = "Horas inválidas";
      if (!data.paseos.trim()) newErrors.paseos = "Este campo es requerido";
      if (!data.vacaciones.trim())
        newErrors.vacaciones = "Este campo es requerido";
    }
  ...
  const sendData = async (e) => {
    ...
    const datosParaEnviar = {
      // Campos del modelo SolicitudAdopcion
      ...
      motivoAdopcion: data.motivoAdopcion.trim(), // ✅ CORREGIDO
        ...
        if (error.response.status === 401) {
          ...
          if (responseData.errors) {
            ...
            if (
              ...
              camposConError.some((campo) => ["motivoAdopcion"].includes(campo))
            ) {
              setCurrentStep(2);
            } else if (
              camposConError.some((campo) =>
              ...
              camposConError.some((campo) =>
                ["horasFueraDeCasa", "aptoEconomicamente"].includes(campo)
              )
            ) {
              setCurrentStep(4);
            }
    ...
    return (
      <div className={styles.section}>
            ...
            <div className={styles.autocompleteList}>
              {filteredList.length > 0 ? (
                ...
                <div className="p-2 text-center">
                  No se encontraron animales para{" "}
                  {formType === "transito" ? "tránsito" : "adopción"}
                </div>
              )}
            </div>
          )}
        ...
        {formType === "adopcion" && (
          <div className="d-flex flex-column mb-4">
            ...
            <input
              ...
              className={`${styles.input} ${
                errors.adoptarOtro ? styles.errorInput : ""
              }`}
            />
            {errors.adoptarOtro && (
        ...
        <div className="d-flex flex-column mb-4">
          <label className="title_orange mx-2 mt-3 mb-1">
            ¿Por qué querés{" "}
            {formType === "transito" ? "dar tránsito" : "adoptar"}? *
          </label>
          <textarea
            name="motivoAdopcion"
              ...
              {currentStep === totalSteps ? (
                <button
                  ...
                  className={`px-4 py-2 rounded btn ${
                    formType === "transito" ? "btn-warning" : "btn-success"
                  }`}
                  disabled={isSubmitting}
                >
                  {isSubmitting && (
