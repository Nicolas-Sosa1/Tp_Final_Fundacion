...
const UpdateAnimal =({listaPerros, setListaPerros, logOut, setLogin })=>{
    ...
    const sendData = (e) => {
        e.preventDefault();
        const URL = `http://localhost:8000/api/animals/update/${id}`;

        
        const payload = {
            ...data,
            castrado: data.castrado === "true",
            desparasitado: data.desparasitado === "true"
        };

        axios.put(URL, payload, { headers: { token_user: localStorage.getItem("token_user") } }).then(response => {

                setListaPerros(listaPerros.map(ani =>ani._id === id ? response.data : ani));
                navigate("/home");
            }
        ).catch(e => {
    ...
    const getData =()=>{
        const URL= `http://localhost:8000/api/animals/${id}`
        axios.get(URL, {headers : {token_user : localStorage.getItem("token_user")}}).then(
            response => {
                const animal = response.data.animal;
                setLogin(true)
                setErrors({})
                setData({...animal, vacunas: animal.vacunas.map(v => v._id)
        });
            }
        ).catch((e) => {
                if (e.response?.status === 406) {
    ...
    return(
        <form onSubmit={sendData} className={styles.formContainer}>
            <h1 className={styles.titulo}>Editar Perro</h1>
